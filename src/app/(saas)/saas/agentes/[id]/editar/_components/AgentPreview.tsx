...
import type { Agent, AgentTemplate } from "@/domain/Agentes"
...
export function AgentPreview({ agent, template, customFields }: AgentPreviewProps) {
  ...
  // Función para obtener el valor de un campo usando el mapeo correcto
  const getFieldValue = (fieldName: string, stepId: string) => {
    ...
    return value !== undefined ? value : directValue
  }

  // Función para obtener todos los campos configurados con sus valores
  const getConfiguredFields = () => {
    const configuredFields: Array<{ key: string; name: string; value: unknown; stepName: string }> = []
    
    if (template?.steps) {
      template.steps.forEach((step) => {
        step.fields.forEach((field) => {
          const value = getFieldValue(field.name, step.id)
  ...
  const totalFields = template?.steps?.reduce((total, step) => total + step.fields.length, 0) ?? 0
  const requiredFields = template?.steps?.reduce((total, step) => 
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <Badge variant={agent.isPrincipal ? "default" : "secondary"} className="border-0">
              {agent.isPrincipal ? "Principal" : "Secundario"}
            </Badge>
            <Badge variant={agent.isActive ? "default" : "secondary"} className="border-0">
        ...
        <div className="grid grid-cols-3 gap-4 py-4 border-t border-b border-gray-100">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{template?.steps?.length ?? 0}</div>
        ...
        {template && (
          <div className="space-y-4">
            <h4 className="font-semibold text-gray-900">Template Base</h4>
            <div className="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg">
              ...
              <Badge variant="outline" className="border-0 bg-white">
        ...
        {template?.steps && template.steps.length > 0 ? (
          <div className="space-y-4">
            ...
            {template.steps.map((step, stepIndex) => (
              <div key={step.id} className="bg-gray-50 rounded-lg p-4 space-y-3 border-0">
        ...
        {configuredFields.length > 0 && (
          <div className="space-y-4">
            <h4 className="font-semibold text-gray-900">Campos Configurados</h4>
            <div className="space-y-2">
              {configuredFields.slice(0, 5).map((field) => (
                <div key={field.key} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex-1">
                    <span className="text-sm font-medium text-gray-900">{field.name}</span>
                    <span className="text-xs text-gray-500 ml-2">({field.stepName})</span>
                  </div>
              ...
              {configuredFields.length > 5 && (
                <div className="text-center py-2">
                  <Badge variant="outline" className="border-0 bg-gray-100">
                    +{configuredFields.length - 5} campos más
                  </Badge>
                </div>
              )}
            </div>
