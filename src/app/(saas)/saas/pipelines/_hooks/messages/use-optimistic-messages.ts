...
export function useOptimisticMessages({
  ...
  const addTemporaryMessage = useCallback((message: TemporaryMessage) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[Pipelines-useOptimisticMessages] ➕ Temporal agregado:', message.id)
    }
  }, [])
  ...
  const removeTemporaryMessage = useCallback((messageId: string) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[Pipelines-useOptimisticMessages] ➖ Temporal removido:', messageId)
    }
  }, [])
  ...
  const updateTemporaryMessage = useCallback((messageId: string, updates: Partial<TemporaryMessage>) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[Pipelines-useOptimisticMessages] 🔄 Temporal actualizado:', messageId, updates)
    }
  }, [])
  ...
  const clearTemporaryMessages = useCallback(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[Pipelines-useOptimisticMessages] 🧹 Limpiando todos los temporales')
    }
  }, [])
  ...
  const allMessages = useMemo(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[Pipelines-useOptimisticMessages] 📊 Mensajes:', {
