...
class RealtimeChannelManager {
  ...
  private async _doGetOrCreateChannel(
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Pipelines-RealtimeManager] 📈 Channel refs: ${channelName} = ${currentCount + 1}`)
    }
    ...
    if (existing) {
      ...
      if (state === 'joined') {
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Pipelines-RealtimeManager] ♻️ Reutilizando canal activo: ${channelName}`)
        }
        return existing
      }

      console.warn(`[Pipelines-RealtimeManager] 🔄 Canal en estado ${state}, limpiando antes de crear: ${channelName}`)
      // Limpiar el canal antiguo de forma segura
      try {
        const supabase = getSupabaseClient()
        await existing.unsubscribe().catch(err => {
          console.warn(`[Pipelines-RealtimeManager] warn: unsubscribe failed for ${channelName}`, err)
        })
        await supabase.removeChannel(existing).catch(err => {
          console.warn(`[Pipelines-RealtimeManager] warn: removeChannel failed for ${channelName}`, err)
        })
      } catch (error) {
        console.error(`[Pipelines-RealtimeManager] Error limpiando canal obsoleto:`, error)
      } finally {
    ...
    if (this.channels.size >= this.MAX_CHANNELS_HARD_LIMIT) {
      ...
      throw new Error(`[Pipelines-RealtimeManager] CRITICAL: Reached hard limit of ${this.MAX_CHANNELS_HARD_LIMIT} channels`)
    }
  ...
  private async _createChannel(
    ...
    console.log(`[Pipelines-RealtimeManager] 🆕 Creando nuevo canal: ${channelName}`)
    const supabase = getSupabaseClient()
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Pipelines-RealtimeManager] ✅ Canal conectado y registrado: ${channelName}`)
      console.log(`[Pipelines-RealtimeManager] 📊 Canales activos: ${this.channels.size}`)
    }
  ...
  private async _doRemoveChannel(channelName: string): Promise<void> {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Pipelines-RealtimeManager] 📉 Channel refs: ${channelName} = ${newCount}`)
    }

    // Only cleanup when no more refs
    if (newCount > 0) {
      if (process.env.NODE_ENV === 'development') {
        console.log(`[Pipelines-RealtimeManager] ⏳ Channel still in use: ${channelName} (${newCount} refs)`)
      }
      return
    }
    ...
    if (!channel) {
      console.log(`[Pipelines-RealtimeManager] ⚠️ Canal no encontrado para remover: ${channelName}`)
      this.subscriptionCounts.delete(channelName)
      return
    }

    this.pendingCleanups.add(channelName)
    console.log(`[Pipelines-RealtimeManager] 🧹 Iniciando limpieza de canal: ${channelName}`)

    try {
      const supabase = getSupabaseClient()

      console.log(`[Pipelines-RealtimeManager] 📤 Unsubscribing canal: ${channelName}`)
      ...
      console.log(`[Pipelines-RealtimeManager] 🗑️ Removiendo canal del cliente: ${channelName}`)
      ...
      console.log(`[Pipelines-RealtimeManager] ✅ Canal limpiado exitosamente: ${channelName}`)
      console.log(`[Pipelines-RealtimeManager] 📊 Canales restantes: ${this.channels.size}`)

    } catch (error) {
      console.error(`[Pipelines-RealtimeManager] ❌ Error limpiando canal ${channelName}:`, error)
      this.channels.delete(channelName)
      this.subscriptionCounts.delete(channelName)
    } finally {
  ...
  async cleanupAll(): Promise<void> {
    console.log(`[Pipelines-RealtimeManager] 🧹 Limpieza total de ${this.channels.size} canales`)
    ...
    console.log('[Pipelines-RealtimeManager] ✅ Limpieza total completada')
  }

  async forceCleanupAll(): Promise<void> {
    console.warn('[Pipelines-RealtimeManager] ⚠️ Force cleaning all channels')
  ...
  private decrementRefCount(channelName: string) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Pipelines-RealtimeManager] 🔽 Decrement ref: ${channelName} => ${next}`)
    }
  }
}
