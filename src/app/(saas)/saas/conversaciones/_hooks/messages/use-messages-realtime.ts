...
export function useMessagesRealtime({
  ...
  useEffect(() => {
    ...
    const setupChannel = async () => {
      try {
        if (process.env.NODE_ENV === 'development') {
          console.log('[useMessagesRealtime] 🚀 Suscribiendo canal:', channelName)
        }
          ...
          (ch) => ch
            ...
            .on(
              ...
              (payload) => {
                ...
                if (process.env.NODE_ENV === 'development') {
                  console.log('[useMessagesRealtime] ➕ Nuevo mensaje realtime:', newMessage.id)
                }
            ...
            .on(
              ...
              (payload) => {
                ...
                if (process.env.NODE_ENV === 'development') {
                  console.log('[useMessagesRealtime] 🔄 Mensaje actualizado:', updatedMessage.id)
                }
            ...
            .on(
              ...
              (payload) => {
                ...
                if (process.env.NODE_ENV === 'development') {
                  console.log('[useMessagesRealtime] 🗑️ Mensaje eliminado:', deletedMessageId)
                }
            ...
            .subscribe((status) => {
              ...
              if (process.env.NODE_ENV === 'development') {
                console.log('[useMessagesRealtime] 📡 Canal:', status)
              }
        ...
        console.error('[useMessagesRealtime] ❌ Error canal:', error)
    ...
    return () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('[useMessagesRealtime] 🧹 Cleanup canal:', channelName)
      }
  ...
  const reconnect = useCallback(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('[useMessagesRealtime] 🔄 Forzando reconexión...')
    }
